	...
	const std::string& abs_filepath, const nlohmann::json& json_data)
{
	...
	// texcoords are the same, just memcpy those--glm's vector types are garunteed to have no padding
	...
	for(auto faceid = 0ull; faceid < mesh_ref.num_face_vertices.size(); ++faceid) {
		assert(mesh_ref.num_face_vertices[faceid] == 3); // shoudln't ever fail, triangulate is turned on
