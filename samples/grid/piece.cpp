...
std::vector<std::vector<piece*>> piece::squares_in_direction(
	glm::ivec2 myLocation, Directions direction, int range)
{
	...
	for (int x = 0; x < range; x++) {
		if (direction == Directions::NORTH) checkLocation.y++;
		if (direction == Directions::WEST) checkLocation.x--;
		if (direction == Directions::SOUTH) checkLocation.y--;
		if (direction == Directions::EAST) checkLocation.x++;
		squares[x] = m_grid->get_actors_from_coord(glm::ivec3(checkLocation.x, checkLocation.y, 2));
	}
	return squares;
}
glm::ivec2 piece::get_location_from_direction(
	glm::ivec3 myLocation, Directions direction, int Length)
{
	glm::ivec2 finalDirection(myLocation.x, myLocation.y);
	switch (direction) {
	case NORTH: finalDirection.y = finalDirection.y + Length; break;
	case WEST: finalDirection.x = finalDirection.x - Length; break;
	case SOUTH: finalDirection.y = finalDirection.y - Length; break;
