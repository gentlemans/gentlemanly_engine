	...
	Rocket::Core::Vector2i& texture_dimensions, const Rocket::Core::String& source)
{
	try {
		...
		texture_dimensions = Rocket::Core::Vector2i(texasset.data->size.x, texasset.data->size.y);
		
		// make our own version because shared_ptr will delete it
		auto tex = new texture{};
		tex->size = texasset.data->size;
		tex->texture_name = texasset.data->texture_name;
