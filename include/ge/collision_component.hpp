...
struct collison_component : public anax::Component
{
	...
	collison_component(transform_component& trans, InputIterator begin, InputIterator end)
	{
		...
		static_assert(std::is_same<std::remove_const_t<std::decay_t<decltype(*begin)>>, glm::vec2>::value, "collison_component::collison_component custom poly constructor expects an InputIterator that dereferences to a glm::vec2");
		
	auto entity_loc = trans.get_transform().location;
