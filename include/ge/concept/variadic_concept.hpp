...
/**
 * A concept (to be used with BOOST_CONCEPT_ASSERT((...))) that applys a single unary concept to a varaidc template (as many types as you want!!)
 * 
 * The first template parameter is actually a template template that takes in a unary concept. 
 * The second parameter is a varaidic template that is the types to be checked. This will give a compile error
