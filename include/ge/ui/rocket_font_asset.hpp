...
namespace ge{
	namespace ui{
		...
		static void load_asset(asset_manager& manager, const char* asset_name,
		const char* filepath, const nlohmann::json& json_data) {
			
			std::string abs_path = boost::filesystem::absolute(json_data["font_file"], filepath).string();
			...
			if(rcss_name_iter == json_data.end()) {
				...
				if(font_style_str == "normal") {
					...
					throw std::runtime_error("Unrecognized font_style \"" + font_style_str + "\" in asset: " + filepath);
				}
				...
				if(font_weight_str == "normal") {
					...
					throw std::runtime_error("Unrecognized font_weight \"" + font_weight_str + "\" in asset: " + filepath);
				}
				
				Rocket::Core::FontDatabase::LoadFontFace(abs_path.c_str(), rcss_name_str.c_str(), e_style, e_weight);
